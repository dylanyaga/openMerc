...
void *conv_load(int nr, int *xs, int *ys)
{
	...
	*ys = gfx[nr].ys;
	...
	for (n = 0; n<size; n++)
	{
		...
		{
		case  1:
			ptr[n] = (unsigned short)((r << 10) | (g2 << 5) | (b));
			break;
		case  2:
			ptr[n] = (unsigned short)((r) | (g1 << 5) | (b << 11));
			break;
		}
	}
...
int create_pnglib(HWND hwnd)
{
	...
	chdir("gfx");
	if (!findfirst("*.png", &ff, FA_ARCH))
	{
		do
		{
			nr = atoi(ff.ff_name);
			if (nr<0 || nr>=MAXSPRITE)
			{
				continue;
			}
...
int init_pnglib(void)
{
	...
	for (n = 0; n<MAXSPRITE; n++)
	{
		if (read(hidx, idx + n, sizeof(int))<sizeof(int))
		{
			break;
		}
	}
