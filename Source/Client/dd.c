...
char *get_dderr(long err)
{
	...
	case DDERR_INCOMPATIBLEPRIMARY:
		ptr = "INCOMPATIBLEPRIMARY";
		break;
	case DDERR_INVALIDCAPS:
		ptr = "INVALIDCAPS";
		break;
	case DDERR_INVALIDPIXELFORMAT:
		ptr = "INVALIDPIXELFORMAT";
		break;
	case DDERR_NOALPHAHW:
		ptr = "NOALPHAHW";
		break;
	case DDERR_NOCOOPERATIVELEVELSET:
		ptr = "NOCOOPERATIVELEVELSET";
		break;
	...
	case DDERR_OUTOFVIDEOMEMORY:
		ptr = "OUTOFVIDEOMEMORY";
		break;
	case DDERR_PRIMARYSURFACEALREADYEXISTS:
		ptr = "PRIMARYSURFACEALREADYEXISTS";
		break;
...
int dd_init(HWND hwnd, int x, int y)
{
	...
	else if (RED==0x7C00 && GREEN==0x03E0 && BLUE==0x001F)
	{
		RGBM = 1;
	}
	else if (RED==0x001F && GREEN==0x07E0 && BLUE==0xF800)
	{
		RGBM = 2;
	}
...
int dd_set_background(int color)
{
	...
	if (ret!=DD_OK)
	{
		dd_error(0, "dd_set_background", ret);
		return( -1);
	}
...
void dd_alphaeffect_magic_1(int nr, int str, int xpos, int ypos, int xoff, int yoff)
{
	...
	for (y = 0; y<64; y++)
	{
		if (dsty + y<200 || dsty + y>=MODEY)
		{
			continue;
		}
		for (x = 0; x<64; x++)
		{
			if (dstx + x<0 || dstx + x>=MODEX)
			{
				continue;
			}
...
void dd_alphaeffect_magic(int nr, int str, int xpos, int ypos, int xoff, int yoff)
{
	if (RGBM==1)
	{
		dd_alphaeffect_magic_1(nr, str, xpos, ypos, xoff, yoff);
	}
	else
	{
		dd_alphaeffect_magic_0(nr, str, xpos, ypos, xoff, yoff);
	}
}
...
void *load_file(char *file)
{
	...
	if (locks)
	{
		xlog(0, "trying to read while locked");
	}
...
void *dd_load_bitmap(char *name, int *xs, int *ys, LPDIRECTDRAWSURFACE sur)
{
	...
	*ys = info->bmiHeader.biHeight;
	...
	if (hbm==0)
	{
		...
		sur->lpVtbl->ReleaseDC(sur, hdc);
		DeleteDC(hdcMem);
		xlog(0, "cannot create hbm (%d)", GetLastError());
		return( NULL);
	}
	...
	while (usedmem + 256 * 1024>maxmem)
	{
		...
		for (n = 0; n<MAXSPRITE; n++)
		{
			...
			if (tmp>old)
			{
				...
				xfree(sprtab[n].cache);
				blockgc++;
				xfree(sprtab[n].image);
				blockgc++;
...
void dd_init_sprites(void)
{
	...
	if (!sprtab)
	{
		sprtab = xcalloc(MAXSPRITE, sizeof(struct sprtab));
	}
	if (!cachetab)
	{
		cachetab = xcalloc(MAXCACHE, sizeof(struct cachetab));
	}
...
int tile2cache(int tile, int sprite, int xpos, int ypos, int xs, int effect)
{
	...
	for (y = 0; y<32; y++)
	{
		for (x = 0; x<32; x++)
		{
			if ((*screen = do_effect(*image, effect, x, y, sprite))!=background)
			{
				visible = 1;
			}
			image++;
...
void copyspritex(int nr, int xpos, int ypos, int effect)
{
	...
	if (sprtab[nr].alpha)
	{
		display_alpha(sprtab[nr].alpha, sprtab[nr].alphacnt, xpos, ypos, effect);
	}
}
...
void dd_savescreen(void)
{
	...
	if (!SetBitmapBits(hbm, MODEX * MODEY * 2, pix))
	{
		return;
	}
...
void dd_gputtext(int xpos, int ypos, int font, char *text, int xoff, int yoff)
{
	...
	while (*text)
	{
		if (rx<0 || rx>MODEX - 7 || ry<0 || ry>MODEY - 10)
		{
			return;
		}
...
void display_alpha(unsigned char *alpha, int alphacnt, int xf, int yf, int effect)
{
	...
	for (n = 0; n<alphacnt; n += 6)
	{
		...
		if (RGBM==0)
		{
			dst[pos] = (unsigned short)(((r << 8) & RED) + ((g << 3) & GREEN) + ((b >> 3) & BLUE));
		}
		else
		{
			dst[pos] = (unsigned short)(((r << 7) & RED) + ((g << 2) & GREEN) + ((b >> 3) & BLUE));
		}
...
void display_png(unsigned char *png, int xf, int yf, int xs, int ys)
{
	...
	for (y = 0; y<ys; y++)
	{
		for (x = 0; x<xs; x++)
		{
			...
			if (RGBM==0)
			{
				dst[x + xf + y * MAXX + yf * MAXX] = (unsigned short)(((r << 8) & RED) + ((g << 3) & GREEN) + ((b >> 3) & BLUE));
			}
			else
			{
				dst[x + xf + y * MAXX + yf * MAXX] = (unsigned short)(((r << 7) & RED) + ((g << 2) & GREEN) + ((b >> 3) & BLUE));
			}
		}
	}
...
unsigned short *dd_load_png(FILE *fp, int *xs, int *ys, unsigned char **alpha_ptr, int *alphacnt_ptr)
{
	...
	for (n = 0; n<y; n++)
	{
		for (m = 0; m<x; m++)
		{
			if (noalpha)
			{
				if (row[n][m * mul]>250 && row[n][m * mul + 1]<5 && row[n][m * mul + 2]>250)
				{
					a = 0;
				}
				else
			...
			if ((do_alpha==0 && a<64) ||
			    ...
			    (do_alpha==2 && a<255))
			{
				bmp[m + n * x] = (unsigned short)(RED | BLUE);
			}
			else if (RGBM==0)
			{
				bmp[m + n * x] = (unsigned short)(((r << 8) & RED) + ((g << 3) & GREEN) + ((b >> 3) & BLUE));
			}
			else
			{
				bmp[m + n * x] = (unsigned short)(((r << 7) & RED) + ((g << 2) & GREEN) + ((b >> 3) & BLUE));
			}
			...
			if (!noalpha && ((do_alpha==1 && a>63 && a<192) || (do_alpha==2 && a!=0 && a!=255)))
			{
				...
				*(unsigned char*)(alpha + alphacnt) = (unsigned char)m;
				alphacnt += 1;
				*(unsigned char*)(alpha + alphacnt) = (unsigned char)n;
				alphacnt += 1;
				*(unsigned char*)(alpha + alphacnt) = r;
				alphacnt += 1;
				*(unsigned char*)(alpha + alphacnt) = g;
				alphacnt += 1;
				*(unsigned char*)(alpha + alphacnt) = b;
				alphacnt += 1;
				*(unsigned char*)(alpha + alphacnt) = a;
				alphacnt += 1;
...
void dd_shadow_clear(void)
{
	if (!shadowmap)
	{
		shadowmap = calloc(MAXX, MAXY);
	}
	else
	{
		memset(shadowmap, 0, MAXX * MAXY);
	}
}
	...
	for (y = ry + ys * 8, m = 0; y>ry; y--, m += xs * 96)
	{
		for (x = rx; x<rx + xs * 32; x++, m++)
		{
			if (src[m]!=(RED | BLUE))
			{
				...
				if (swap==1)
				{
					p = x + y * MAXX + (int)((x - rx) * 0.885) * MAXX;
				}
				else if (swap==2)
				{
					p = x + y * MAXX - (int)((x - rx) * 0.885) * MAXX;
				...
				if (RGBM==0)
				{
					v &= (0x7800 | 0x03E0 | 0x000F);
				}
				else
				{
					v &= (0x3C00 | 0x01E0 | 0x000F);
				}
				dst[p] = v;
