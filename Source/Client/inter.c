...
int trans_button(int x, int y)
{
	...
	if (x>290 && y>1 && x<300 && y<34)
	{
		return( 12);
	}
	if (x>290 && y>141 && x<300 && y<174)
	{
		return( 13);
	}

	if (x>206 && y>113 && x<218 && y<148)
	{
		return( 14);
	}
	if (x>206 && y>218 && x<218 && y<252)
	{
		return( 15);
	}
...
void button_help(int nr)
{
	...
	{
	case  0:
		xlog(1, "F1/FAST: Makes you move faster, but takes a lot more endurance. You will also be seen more easily.");
		break;
	case  1:
	...
	case  2:
		...
		break;
	case  3:
		xlog(1, "F4/HEALTH: Toggle whether the current health of all characters is displayed.");
		break;
	case  4:
		break;
	case  5:
	...
	case  7:
		break;
	case  11:
		xlog(1, "F12/EXIT: Leave the game. Will cost you 50%% health and 10%% gold.");
		break;                                                                                // exit

	case  12:
		xlog(1, "Scroll inventory contents up.");
		break;
	case  13:
		xlog(1, "Scroll inventory contents down");
		break;

	case  14:
		xlog(1, "Scroll skill list up");
		break;
	case  15:
		xlog(1, "Scroll skill list down");
		break;
...
void reset_block(void)
{
	...
	if (pl.citem)
	{
		...
		if (pl.citem_p & PL_WEAPON)
		{
			inv_block[WN_RHAND] = 0;
		}
		else
		{
			inv_block[WN_RHAND] = 1;
		}
		if (pl.citem_p & PL_SHIELD)
		{
			inv_block[WN_LHAND] = 0;
		}
		else
		{
			inv_block[WN_LHAND] = 1;
		}
		if (pl.citem_p & PL_CLOAK)
		{
			inv_block[WN_CLOAK] = 0;
		}
		else
		{
			inv_block[WN_CLOAK] = 1;
		}
		if (pl.citem_p & PL_RING)
		{
			inv_block[WN_LRING] = inv_block[WN_RRING] = 0;
		}
		else
		{
			inv_block[WN_LRING] = inv_block[WN_RRING] = 1;
		}
	}
	else
	{
	...
	if (pl.worn_p[WN_RHAND] & PL_TWOHAND)
	{
		inv_block[WN_LHAND] = 1;
	}
}
...
int mouse_inventory(int x, int y, int mode)
{
	...
	if (y>176 && y<203)
	{
		if (x>219 && x<246)
		{
			if (mode==MS_LB_UP)
			{
				cmd3(CL_CMD_INV, 2, 1, selected_char);
			}
			if (mode==MS_RB_UP)
			{
				xlog(1, "1 silver coin.");
			}
			hightlight = HL_MONEY;
			hightlight_sub = 1;
			cursor_type = CT_TAKE;
		...
		if (x>247 && x<274)
		{
			if (mode==MS_LB_UP)
			{
				cmd3(CL_CMD_INV, 2, 10, selected_char);
			}
			if (mode==MS_RB_UP)
			{
				xlog(1, "10 silver coins.");
			}
			hightlight = HL_MONEY;
			hightlight_sub = 2;
			cursor_type = CT_TAKE;
		...
		if (x>275 && x<301)
		{
			if (mode==MS_LB_UP)
			{
				cmd3(CL_CMD_INV, 2, 100, selected_char);
			}
			if (mode==MS_RB_UP)
			{
				xlog(1, "1 gold coin.");
	...
	if (y>205 && y<231)
	{
		if (x>219 && x<246)
		{
			if (mode==MS_LB_UP)
			{
				cmd3(CL_CMD_INV, 2, 1000, selected_char);
			}
			if (mode==MS_RB_UP)
			{
				xlog(1, "10 gold coins.");
			}
			hightlight = HL_MONEY;
			hightlight_sub = 4;
			cursor_type = CT_TAKE;
		...
		if (x>247 && x<274)
		{
			if (mode==MS_LB_UP)
			{
				cmd3(CL_CMD_INV, 2, 10000, selected_char);
			}
			if (mode==MS_RB_UP)
			{
				xlog(1, "100 gold coins.");
			}
			hightlight = HL_MONEY;
			hightlight_sub = 5;
			cursor_type = CT_TAKE;
		...
		if (x>275 && x<301)
		{
			if (mode==MS_LB_UP)
			{
				cmd3(CL_CMD_INV, 2, 100000, selected_char);
			}
			if (mode==MS_RB_UP)
			{
				xlog(1, "1,000 gold coins.");
			}
			hightlight = HL_MONEY;
			hightlight_sub = 1;
			cursor_type = CT_TAKE;
	...
	if (y>232 && y<259 && x>219 && x<246)
	{
		if (mode==MS_LB_UP)
		{
			cmd3(CL_CMD_INV, 2, 1000000, selected_char);
		}
		if (mode==MS_RB_UP)
		{
			xlog(1, "10,000 gold coins.");
		}
		hightlight = HL_MONEY;
		hightlight_sub = 2;
		cursor_type = CT_TAKE;
	...
	if (x>219 && x<288 && y>1 && y<175)
	{
		...
		if (keys==1)
		{
			if (mode==MS_LB_UP)
			{
				cmd3(CL_CMD_INV, 0, nr + inv_pos, selected_char);
			}
			else if (mode==MS_RB_UP)
			{
				cmd3(CL_CMD_INV_LOOK, nr + inv_pos, 0, selected_char);
			}
		...
		else if (keys==0)
		{
			if (mode==MS_LB_UP)
			{
				cmd3(CL_CMD_INV, 6, nr + inv_pos, selected_char);
			}
			else if (mode==MS_RB_UP)
			{
				cmd3(CL_CMD_INV_LOOK, nr + inv_pos, 0, selected_char);
	...
	if (x>302 && x<371 && y>1 && y<175 + 35)
	{
		...
		if (tx==0 && ty==2)   // neck
		{
			if (keys==1)
			{
				if (mode==MS_LB_UP)
				{
					cmd3(CL_CMD_INV, 1, 1, selected_char);
				}
				else if (mode==MS_RB_UP)
				{
					cmd3(CL_CMD_INV, 7, 1, selected_char);
				}
				if (pl.worn[1])
				{
					if (pl.citem)
			...
			else if (keys==0)
			{
				if (mode==MS_LB_UP)
				{
					cmd3(CL_CMD_INV, 5, 1, selected_char);
				}
				else if (mode==MS_RB_UP)
				{
					cmd3(CL_CMD_INV, 7, 1, selected_char);
		...
		if (tx==0 && ty==0)   // head
		{
			if (keys==1)
			{
				if (mode==MS_LB_UP)
				{
					cmd3(CL_CMD_INV, 1, 0, selected_char);
				}
				else if (mode==MS_RB_UP)
				{
					cmd3(CL_CMD_INV, 7, 0, selected_char);
				}
				if (pl.worn[0])
				{
					if (pl.citem)
			...
			else if (keys==0)
			{
				if (mode==MS_LB_UP)
				{
					cmd3(CL_CMD_INV, 5, 0, selected_char);
				}
				else if (mode==MS_RB_UP)
				{
					cmd3(CL_CMD_INV, 7, 0, selected_char);
		...
		if (tx==1 && ty==1)   // arms
		{
			if (keys==1)
			{
				if (mode==MS_LB_UP)
				{
					cmd3(CL_CMD_INV, 1, 3, selected_char);
				}
				else if (mode==MS_RB_UP)
				{
					cmd3(CL_CMD_INV, 7, 3, selected_char);
				}
				if (pl.worn[3])
				{
					if (pl.citem)
			...
			else if (keys==0)
			{
				if (mode==MS_LB_UP)
				{
					cmd3(CL_CMD_INV, 5, 3, selected_char);
				}
				else if (mode==MS_RB_UP)
				{
					cmd3(CL_CMD_INV, 7, 3, selected_char);
		...
		if (tx==1 && ty==3)   // lhand
		{
			if (keys==1)
			{
				if (mode==MS_LB_UP)
				{
					cmd3(CL_CMD_INV, 1, 7, selected_char);
				}
				else if (mode==MS_RB_UP)
				{
					cmd3(CL_CMD_INV, 7, 7, selected_char);
				}
				if (pl.worn[7])
				{
					if (pl.citem)
			...
			else if (keys==0)
			{
				if (mode==MS_LB_UP)
				{
					cmd3(CL_CMD_INV, 5, 7, selected_char);
				}
				else if (mode==MS_RB_UP)
				{
					cmd3(CL_CMD_INV, 7, 7, selected_char);
		...
		if (tx==0 && ty==4)   // lring
		{
			if (keys==1)
			{
				if (mode==MS_LB_UP)
				{
					cmd3(CL_CMD_INV, 1, 11, selected_char);
				}
				else if (mode==MS_RB_UP)
				{
					cmd3(CL_CMD_INV, 7, 11, selected_char);
				}
				if (pl.worn[11])
				{
					if (pl.citem)
			...
			else if (keys==0)
			{
				if (mode==MS_LB_UP)
				{
					cmd3(CL_CMD_INV, 5, 11, selected_char);
				}
				else if (mode==MS_RB_UP)
				{
					cmd3(CL_CMD_INV, 7, 11, selected_char);
		...
		if (tx==1 && ty==4)   // rring
		{
			if (keys==1)
			{
				if (mode==MS_LB_UP)
				{
					cmd3(CL_CMD_INV, 1, 10, selected_char);
				}
				else if (mode==MS_RB_UP)
				{
					cmd3(CL_CMD_INV, 7, 10, selected_char);
				}
				if (pl.worn[10])
				{
					if (pl.citem)
			...
			else if (keys==0)
			{
				if (mode==MS_LB_UP)
				{
					cmd3(CL_CMD_INV, 5, 10, selected_char);
				}
				else if (mode==MS_RB_UP)
				{
					cmd3(CL_CMD_INV, 7, 10, selected_char);
		...
		if (tx==0 && ty==5)   // legs
		{
			if (keys==1)
			{
				if (mode==MS_LB_UP)
				{
					cmd3(CL_CMD_INV, 1, 5, selected_char);
				}
				else if (mode==MS_RB_UP)
				{
					cmd3(CL_CMD_INV, 7, 5, selected_char);
				}
				if (pl.worn[5])
				{
					if (pl.citem)
			...
			else if (keys==0)
			{
				if (mode==MS_LB_UP)
				{
					cmd3(CL_CMD_INV, 5, 5, selected_char);
				}
				else if (mode==MS_RB_UP)
				{
					cmd3(CL_CMD_INV, 7, 5, selected_char);
		...
		if (tx==1 && ty==5)   // feet
		{
			if (keys==1)
			{
				if (mode==MS_LB_UP)
				{
					cmd3(CL_CMD_INV, 1, 6, selected_char);
				}
				else if (mode==MS_RB_UP)
				{
					cmd3(CL_CMD_INV, 7, 6, selected_char);
				}
				if (pl.worn[6])
				{
					if (pl.citem)
			...
			else if (keys==0)
			{
				if (mode==MS_LB_UP)
				{
					cmd3(CL_CMD_INV, 5, 6, selected_char);
				}
				else if (mode==MS_RB_UP)
				{
					cmd3(CL_CMD_INV, 7, 6, selected_char);
		...
		if (tx==1 && ty==0)   // cloak
		{
			if (keys==1)
			{
				if (mode==MS_LB_UP)
				{
					cmd3(CL_CMD_INV, 1, 9, selected_char);
				}
				else if (mode==MS_RB_UP)
				{
					cmd3(CL_CMD_INV, 7, 9, selected_char);
				}
				if (pl.worn[9])
				{
					if (pl.citem)
			...
			else if (keys==0)
			{
				if (mode==MS_LB_UP)
				{
					cmd3(CL_CMD_INV, 5, 9, selected_char);
				}
				else if (mode==MS_RB_UP)
				{
					cmd3(CL_CMD_INV, 7, 9, selected_char);
		...
		if (tx==0 && ty==3)   // rhand
		{
			if (keys==1)
			{
				if (mode==MS_LB_UP)
				{
					cmd3(CL_CMD_INV, 1, 8, selected_char);
				}
				else if (mode==MS_RB_UP)
				{
					cmd3(CL_CMD_INV, 7, 8, selected_char);
				}
				if (pl.worn[8])
				{
					if (pl.citem)
			...
			else if (keys==0)
			{
				if (mode==MS_LB_UP)
				{
					cmd3(CL_CMD_INV, 5, 8, selected_char);
				}
				else if (mode==MS_RB_UP)
				{
					cmd3(CL_CMD_INV, 7, 8, selected_char);
		...
		if (tx==1 && ty==2)   // belt
		{
			if (keys==1)
			{
				if (mode==MS_LB_UP)
				{
					cmd3(CL_CMD_INV, 1, 4, selected_char);
				}
				else if (mode==MS_RB_UP)
				{
					cmd3(CL_CMD_INV, 7, 4, selected_char);
				}
				if (pl.worn[4])
				{
					if (pl.citem)
			...
			else if (keys==0)
			{
				if (mode==MS_LB_UP)
				{
					cmd3(CL_CMD_INV, 5, 4, selected_char);
				}
				else if (mode==MS_RB_UP)
				{
					cmd3(CL_CMD_INV, 7, 4, selected_char);
		...
		if (tx==0 && ty==1)   // body
		{
			if (keys==1)
			{
				if (mode==MS_LB_UP)
				{
					cmd3(CL_CMD_INV, 1, 2, selected_char);
				}
				else if (mode==MS_RB_UP)
				{
					cmd3(CL_CMD_INV, 7, 2, selected_char);
				}
				if (pl.worn[2])
				{
					if (pl.citem)
			...
			else if (keys==0)
			{
				if (mode==MS_LB_UP)
				{
					cmd3(CL_CMD_INV, 5, 2, selected_char);
				}
				else if (mode==MS_RB_UP)
				{
					cmd3(CL_CMD_INV, 7, 2, selected_char);
...
int _mouse_statbox(int x, int y, int state)
{
	...
	if (x>109 && y>254 && x<158 && y<266)
	{
		...
		if (state==MS_RB_UP)
		{
			xlog(1, "Make the changes permanent");
		}
		if (state!=MS_LB_UP)
		{
			return( 1);
	...
	if (x<145)   // raise
	{
		if (state==MS_RB_UP)
		{
			if (n<5)
			{
				xlog(1, "Raise %s.", at_name[n]);
			}
			else if (n==5)
			{
				xlog(1, "Raise Hitpoints.");
			}
			else if (n==6)
			{
				xlog(1, "Raise Endurance.");
			}
			else if (n==7)
			{
				xlog(1, "Raise Mana.");
			...
			{
				xlog(1, "Raise %s.", skilltab[n - 8 + skill_pos].name);
			}
			return(1);
		}
		if (state!=MS_LB_UP)
		...
		{
			if (hp_needed(pl.hp[0] + stat_raised[n])>pl.points - stat_points_used)
			{
				return( 1);
			}
		...
		{
			if (end_needed(pl.end[0] + stat_raised[n])>pl.points - stat_points_used)
			{
				return( 1);
			}
		...
		{
			if (mana_needed(pl.mana[0] + stat_raised[n])>pl.points - stat_points_used)
			{
				return( 1);
			}
		...
		else
		{
			...
			if (skill_needed(m, pl.skill[m][0] + stat_raised[n + skill_pos])>pl.points - stat_points_used)
			{
				return( 1);
			}
	...
	else     // lower
	{
		if (state==MS_RB_UP)
		{
			if (n<5)
			{
				xlog(1, "Lower %s.", at_name[n]);
			}
			else if (n==5)
			{
				xlog(1, "Lower Hitpoints.");
			}
			else if (n==6)
			{
				xlog(1, "Lower Endurance.");
			}
			else if (n==7)
			{
				xlog(1, "Lower Mana.");
			...
			{
				xlog(1, "Lower %s.", skilltab[n - 8 + skill_pos].name);
			}
			return(1);
		}
		if (state!=MS_LB_UP)
...
void mouse_mapbox(int x, int y, int state)
{
	...
	if (mx<3 * 32 + 12 || mx>(TILEX - 7) * 32 + 20 || my<7 * 32 + 12 || my>(TILEY - 3) * 32 + 20)
	{
		if (state==MS_LB_UP)
		{
			...
			if (my==17 && (mx==27 || mx==28))
			{
				xmove = 3;
			}
			if (mx==17 && (my==31 || my==32))
			{
				xmove = 4;
			}
		}
...
//  xlog("ch_sprite=%d, nr=%d, id=%d",map[m].ch_sprite,map[m].ch_nr,map[m].ch_id);
	...
	if (keys==0)
	{
		...
		{
			xmove = 0;
			cmd(CL_CMD_TURN, map[m].x, map[m].y);
		}
		if (state==MS_LB_UP)
		{
			xmove = 0;
	...
	if (keys==1)
	{
		...
		else if (map[m + 1 - TILEX].flags & ISITEM)
		{
			mx++;
			my--;
		}
		else if (map[m + 2 - 2 * TILEX].flags & ISITEM)
		{
			mx += 2;
			my -= 2;
		}
		else if (map[m + 1].flags & ISITEM)
		{
			mx++;
		}
		else if (map[m + TILEX].flags & ISITEM)
		{
			my++;
		}
		else if (map[m - 1].flags & ISITEM)
		...
		else if (map[m + 2].flags & ISITEM)
		{
			mx += 2;
		}
		else if (map[m + 2 * TILEX].flags & ISITEM)
		{
			my += 2;
		}
		else if (map[m - 2].flags & ISITEM)
		...
		else if (map[m + 1 + 2 * TILEX].flags & ISITEM)
		{
			mx++;
			my += 2;
		}
		...
		else if (map[m + 2 + 1 * TILEX].flags & ISITEM)
		{
			mx += 2;
			my++;
		}
		...
		if ((map[m].flags & ISITEM))
		{
			if (state==MS_LB_UP)
			{
				...
				{
					xmove = 0;
					cmd(CL_CMD_USE, map[m].x, map[m].y);
					noshop = 0;
				}
				else
				{
			...
			{
				xmove = 0;
				cmd(CL_CMD_LOOK_ITEM, map[m].x, map[m].y);
			}
			tile_type = 1;
		}
		return;
	...
	if (keys==2)
	{
		...
		else if (map[m + 1 - TILEX].flags & ISCHAR)
		{
			mx++;
			my--;
			hightlight = HL_MAP;
		}
		else if (map[m + 2 - 2 * TILEX].flags & ISCHAR)
		{
			mx += 2;
			my -= 2;
			hightlight = HL_MAP;
		}
		else if (map[m + 1].flags & ISCHAR)
		{
			mx++;
			hightlight = HL_MAP;
		}
		else if (map[m + TILEX].flags & ISCHAR)
		{
			my++;
			hightlight = HL_MAP;
		}
		...
		else if (map[m + 2].flags & ISCHAR)
		{
			mx += 2;
			hightlight = HL_MAP;
		}
		else if (map[m + 2 * TILEX].flags & ISCHAR)
		{
			my += 2;
			hightlight = HL_MAP;
		}
		...
		else if (map[m + 1 + 2 * TILEX].flags & ISCHAR)
		{
			mx++;
			my += 2;
			hightlight = HL_MAP;
		...
		else if (map[m + 2 + 1 * TILEX].flags & ISCHAR)
		{
			mx += 2;
			my++;
			hightlight = HL_MAP;
		...
		if (map[m].flags & ISCHAR)
		{
			...
			{
				xmove = 0;
				cmd1(CL_CMD_ATTACK, map[m].ch_nr);
			}
			else if (state==MS_RB_UP)
			{
				xmove = 0;
				cmd1(CL_CMD_LOOK, map[m].ch_nr);
				noshop = 0;
			}
			tile_type = 2;
		}
	...
	if (keys==4)
	{
		...
		if (map[m].flags & ISCHAR)
		{
			...
			{
				xmove = 0;
				if (selected_char==map[m].ch_nr)
				{
					selected_char = 0;
				}
				else
				{
					selected_char = map[m].ch_nr;
				}
			}
			else if (state==MS_RB_UP)
			{
