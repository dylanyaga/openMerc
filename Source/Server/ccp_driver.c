...
void ccp_shout(int cn, int co, char *text)
{
	...
	if (mem->lastshout>globs->ticker + TICKS * 10)
	{
		return;
	}
...
int ccp_set_enemy(int cn, int co)
{
	...
	if (ch[co].points_tot>ch[cn].points_tot * 4)
	{
		mem->enemy_strength = 7;
	}
	else if (ch[co].points_tot>ch[cn].points_tot * 2)
	{
		mem->enemy_strength = 6;
	}
	else if (ch[co].points_tot>(ch[cn].points_tot * 3) / 2)
	{
		mem->enemy_strength = 5;
	}
	else if (ch[co].points_tot>ch[cn].points_tot)
	{
		mem->enemy_strength = 4;
	}
	else if (ch[co].points_tot>(ch[cn].points_tot * 2) / 3)
	{
		mem->enemy_strength = 3;
	}
	else if (ch[co].points_tot>ch[cn].points_tot / 2)
	{
		mem->enemy_strength = 2;
	}
	else if (ch[co].points_tot>ch[cn].points_tot / 4)
	{
		mem->enemy_strength = 1;
	}
	else
	{
		mem->enemy_strength = 0;
	}
...
void ccp_seen(int cn, int co)
{
	...
	{
		if (ch[co].points_tot>ch[cn].points_tot)
		{
	...
	if (ch[cn].a_hp<ch[cn].hp[5] * 800)
	{
		return;
	}
	if (ch[cn].a_mana<ch[cn].hp[5] * 800)
	{
		return;
	}
	...
	if (!can_go(ch[cn].x, ch[cn].y, ch[co].x, ch[co].y))
	{
		return;
	}

	if (ch[co].points_tot>(ch[cn].points_tot * 3) / 2)
	{
		return;                                         // too strong
...
void ccp_driver(int cn)
{
	...
	if (ch[cn].a_hp<ch[cn].hp[5] * 500 && !ccp_at_recall_point(cn) && npc_try_spell(cn, cn, SK_RECALL))
	{
		return;
	}
