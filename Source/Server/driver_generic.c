...
void player_driver_med(int cn)
{
	...
	if (ch[cn].data[12] + TICKS * 15>globs->ticker)
	{
		return;
	}
...
void driver(int cn)
{
	...
	{
		prof = prof_start();
		drv_moveto(cn, ch[cn].goto_x, ch[cn].goto_y);
		prof_stop(36, prof);
		return;
	}
	if (ch[cn].attack_cn)
	{
		prof = prof_start();
		drv_attack_char(cn, ch[cn].attack_cn);
		prof_stop(35, prof);
		return;
	}
	...
	case    DR_DROP:
		prof = prof_start();
		drv_dropto(cn, ch[cn].misc_target1, ch[cn].misc_target2);
		prof_stop(28, prof);
		break;
	case    DR_PICKUP:
		prof = prof_start();
		drv_pickupto(cn, ch[cn].misc_target1, ch[cn].misc_target2);
		prof_stop(29, prof);
		break;
	case    DR_GIVE:
		prof = prof_start();
		drv_give_char(cn, ch[cn].misc_target1);
		prof_stop(30, prof);
		break;
	case    DR_USE:
		prof = prof_start();
		drv_useto(cn, ch[cn].misc_target1, ch[cn].misc_target2);
		prof_stop(31, prof);
		break;
	case    DR_BOW:
		prof = prof_start();
	...
	case    DR_TURN:
		prof = prof_start();
		drv_turnto(cn, ch[cn].misc_target1, ch[cn].misc_target2);
		prof_stop(34, prof);
		break;
	case    DR_SINGLEBUILD:
		break;
	...
	case    DR_AREABUILD2:
		break;
	default:
		xlog("player_driver(): unknown misc_action %d", ch[cn].misc_action);
		ch[cn].misc_action = DR_IDLE;
	}
}
...
void driver_msg(int cn, int type, int dat1, int dat2, int dat3, int dat4)
{
	...
	if (!(ch[cn].flags & (CF_PLAYER | CF_USURP)))
	{
		if (npc_msg(cn, type, dat1, dat2, dat3, dat4))
		{
			return;
		}
	}
