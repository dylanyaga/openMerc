...
int astar(int fx, int fy, int cdir)
{
	...
	while (ngo && !failed)
	{
		...
		if (mode==0 && node->x==tx1 && node->y==ty1)
		{
			return( node->dir);
		}
		if (mode!=0 && abs(node->x - tx1) + abs(node->y - ty1)==1)
		{
			return( node->dir);
		}
		if (mode==2 && abs(node->x - tx2) + abs(node->y - ty2)==1)
		{
			return( node->dir);
		}
		add_suc(node);
